...
StackForge uses Clerk for authentication, providing a complete user management system with secure authentication flows. This guide will walk you through configuring authentication for your application.
...
3. Navigate to the "API Keys" section and note your API keys
...
Replace the placeholder values with your actual Clerk API keys from the Clerk dashboard.
...
The sign-in process uses email verification codes rather than traditional password authentication.
...
The sign-up process uses email verification codes rather than traditional password authentication.
...
To customize the sign-in form, modify the component at `src/components/auth/signin-form.tsx`. The form uses email verification with code-based authentication:

```tsx
// Example customization in src/app/auth/signin/page.tsx
...
To customize the sign-up form, modify the component at `src/components/auth/signup-form.tsx`. The form uses email verification with code-based authentication:

```tsx
// Example customization in src/app/auth/signup/page.tsx
...
StackForge supports social authentication providers through Clerk. Currently implemented providers include:
...
The authentication forms in StackForge are designed to work with any social provider supported by Clerk.
