...
StackForge supports two authentication systems: Clerk (default) and Auth.js (NextAuth.js). You only need to configure one of them.

```env
# Clerk Authentication (Optional - remove if using Auth.js)
...
# Auth.js Configuration (Optional - remove if using Clerk)
...
You can validate your environment variables using the built-in validation script:

```bash
bun run env:validate
```

This will check that all required environment variables are present and in the correct format, providing clear error messages if any are missing or invalid.

The validation is also automatically run when the application starts to ensure all required environment variables are properly configured.
